...
class SauerLab:
    def create_binary_image(self, im: np.ndarray) -> np.ndarray:
        """
        ...
        indices = np.where(bin_image)
        #approximate fill factor by space occupied by structure
        self.fill_factor = (indices[0].max()-indices[0].min())/6000*(indices[1].max()-indices[1].min())/6000
        print(bin_image.sum()/max(bin_image.shape)**2)
        return bin_image
