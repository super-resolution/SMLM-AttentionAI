...
class Simulation(nn.Module):
    ...
    def __init__(self, device="cpu"):
        ...
        self.dark_noise =torch.tensor([ 1.], device=device)  # pro sekunde
        ...
        self.psf = lambda mu: torch.sum((1. / torch.sqrt(2. * torch.pi**d) * torch.det(cov)).squeeze() * \
    ...
    def from_dict(cls, camera_dict):
        ...
        for key,value in camera_dict.items():
            if key in instance.ALLOWED_KWARGS:
                #cast value on float
                setattr(instance, key, torch.tensor(value, dtype=torch.float32, device=self.device))
            else:
    ...
    def forward(self, positions):
        ...
        pdf = self.psf(positions)
        pdf = pdf/torch.max(pdf)
        discretepdf = pdf * self.expected_number_of_photons * self.quantum_efficiency
        discretepdf += self.dark_noise * self.exposure_time

        x = td.Poisson(discretepdf)
        #α > 0 constraint
